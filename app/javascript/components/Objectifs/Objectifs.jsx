...
class Objectifs extends React.Component {
    ...
    componentDidMount() {
    const url = "/api/v1/objectifs/index";
    fetch(url)
      .then(response => {
        if (response.ok) {
      ...
      .then(response => this.setState({ objectifs: response.objectifs }))
    ...
    handleSubmit(event) {
        ...
        const url = "/api/v1/objectifs/import";
        ...
        fetch(url, {
          ...
          .then(response => this.setState({ objectifs: response.objectifs, loading: false }))
          .catch(error => console.log(error.message));
    }
      ...
      <li key={file.name}>
        ...
        <div className="pd24">
            ...
            <Row className=" loader_box align_center">
              <Col sm={12} lg={6}>
                <div className="titre_part">Fichier Objectifs</div>
                <div className="d24"></div>
                <div>{this.state.objectifs.length}</div>
              </Col>
              <Col sm={12} lg={6}>

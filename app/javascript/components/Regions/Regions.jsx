...
class Regions extends React.Component {
    ...
    componentDidMount() {
    const url = "/api/v1/regions/index";
    fetch(url)
      .then(response => {
        if (response.ok) {
    ...
    handleSubmit(event) {
        ...
        const url = "/api/v1/regions/import";
        ...
        fetch(url, {
          ...
          .then(response => this.props.history.push(`/regions`))
          .catch(error => console.log(error.message));
    }
      ...
      <li key={file.name}>
        ...
        <div className="pd24">
            ...
            <Row className=" loader_box align_center">
              <Col sm={12} lg={6}>
                <div className="titre_part">Fichier Régions</div>
                <div className="d24"></div>
                {this.state.regions.map((region, index) => (
                <div key={index}>
                  <div className="texte_etiquette">{region.nom}</div>
                </div>
                ))}
              </Col>
              <Col sm={12} lg={6}>
